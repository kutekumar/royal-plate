...
const Food = () => {
  ...
  return (
      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          ...
          <motion.button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all text-[9px] font-bold shadow-md ${
      ...
      >
        <div className="relative group">
          ...
          <Input
            ...
            className="relative w-full h-10 pl-10 pr-9 bg-white/90 backdrop-blur-md border border-white/60 rounded-2xl text-[#1D2956] text-base placeholder-gray-400 focus:border-[#536DFE]/40 focus:ring-4 focus:ring-[#536DFE]/10 shadow-lg shadow-black/5 transition-all"
          />
          {searchQuery && (
            <motion.button
              initial={{ scale: 0 }}
          ...
          >
              ...
              {SORT_OPTIONS.map((opt, index) => (
                <motion.button
                  ...
                  onClick={() => setSortBy(opt.value)}
                   className={`px-3 py-2 rounded-2xl text-[9px] font-bold transition-all shadow-md ${
                    sortBy === opt.value
            ...
            {(selectedCategory !== ALL_ITEMS_CATEGORY || sortBy !== 'default') && (
              <motion.button
                ...
                className="flex items-center gap-1.5 mt-3 text-gray-400 hover:text-[#536DFE] text-[9px] font-bold transition-colors"
              >
      ...
      >
        <div className="flex gap-2 overflow-x-auto px-4 pb-2 scrollbar-hide">
          {categories.map((cat, index) => (
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[9px] font-bold tracking-wide transition-all shadow-md ${
      ...
      <div className="px-4 pb-2 flex-shrink-0 flex items-center justify-between">
        ...
        {totalPages > 1 && (
          <p className="text-gray-400 text-[9px] font-medium">Page {currentPage} of {totalPages}</p>
        )}
      </div>
      ...
      {favoriteItems.length > 0 && (
        <div className="px-4 pb-3 flex-shrink-0">
          <div className="flex items-center gap-1.5 mb-2.5">
            ...
            <span className="text-[#1D2956] text-[9px] font-bold uppercase tracking-[0.15em]">Your Favorites</span>
          </div>
          <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-hide">
            {favoriteItems.slice(0, 8).map((item, idx) => (
              <motion.button
                ...
                className="flex-shrink-0 flex items-center gap-1.5 bg-white/90 backdrop-blur-md rounded-xl px-2.5 py-1.5 border border-white/60 shadow-md hover:shadow-lg hover:border-rose-300 transition-all"
              >
                  <div className="w-6 h-6 rounded-lg overflow-hidden flex-shrink-0">
                    <img src={item.image_url} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
                  </div>
                  <span className="text-[#1D2956] text-[9px] font-bold whitespace-nowrap">{item.name}</span>
              </motion.button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-20 scrollbar-hide" style={{ contentVisibility: 'auto' }}>
        {loading ? (
            ...
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-2xl bg-white/80 backdrop-blur-md overflow-hidden animate-pulse shadow-lg shadow-black/5">
                ...
                <div className="p-2 space-y-1.5">
                  <div className="h-2 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full w-3/4" />
                  <div className="h-2 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full w-1/2" />
                  <div className="h-2 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full w-2/5" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-gray-400 text-[10px]">Try adjusting your search or filters</p>
          ...
          <div className="grid grid-cols-3 gap-2">
            {items.map((item, index) => (
              ...
              >
                ...
                <div className="relative h-24 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 brand-menu-filter brand-shimmer">
                  {item.image_url ? (
                    <img
                      src={item.image_url}
                      alt={item.name}
                ...
                <div className="p-2">
                  <motion.h3
                    ...
                    className="text-[#1D2956] text-[9px] font-bold leading-tight line-clamp-1 mb-0.5 group-hover:text-[#536DFE] transition-colors"
                  >
                    {item.name}
                  </motion.h3>
                  <motion.p
                    ...
                    className="text-gray-400 text-[8px] line-clamp-1 mb-1 font-medium tracking-wide"
                  >
                  ...
                  >
                    <span className="text-[#536DFE] font-bold text-[9px] tracking-tight">{formatPrice(item.price)}</span>
                    <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#536DFE] to-[#6B7FFF] flex items-center justify-center shadow-md shadow-[#536DFE]/30 group-hover:scale-110 transition-transform">
        ...
        {!loading && totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 py-5">
            <button
              ...
              className="flex items-center justify-center w-8 h-8 rounded-xl border border-white/60 bg-white/90 backdrop-blur-md text-[#1D2956] disabled:opacity-30 disabled:cursor-not-allowed hover:border-[#536DFE]/50 hover:bg-gradient-to-br hover:from-[#536DFE]/5 hover:to-[#6B7FFF]/5 transition-all shadow-md shadow-black/5 hover:shadow-lg"
            >
              ...
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 rounded-xl text-[9px] font-bold transition-all shadow-md ${
                      currentPage === page
                        ? 'bg-gradient-to-br from-[#536DFE] to-[#6B7FFF] text-white shadow-[#536DFE]/30 scale-105'
